...
<body>
	<div class="row-fluid">
		...
		<div id="table_report_wrapper" class="dataTables_wrapper" role="grid">
			<form action="student/studentList.do" method="post" name="studentForm"
				id="studentForm">
							...
							<c:when test="${not empty students}">
								<c:forEach items="${students}" var="student" varStatus="vs">
									...
									<td class='center'><label><input type='checkbox' name='ids'
															value="${student.sno }" /><span class="lbl"></span></label></td>
						...
						<table style="width: 100%; margin-left: 30px;">
							<tr>
								<td style="vertical-align: top;"><a
									class="btn btn-small btn-yellow" onclick="addAll('${pd.coursesMessageId}');">添加</a>
								</td>
	...
	<script type="text/javascript">
		...
		//新增
		function addAll(coursesMessageId){
			var str = '';
			var emstr = '';
			var phones = '';
			...
			if(str == ''){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>score/addAllS.do?tm='+new Date().getTime(),
			    	data: {snos:str,coursesMessageId:coursesMessageId},
					dataType:'json',
					cache: false,
