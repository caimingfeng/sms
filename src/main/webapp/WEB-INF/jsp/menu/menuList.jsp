...
<script type="text/javascript">
	$(top.hangge());	
	 $(function() {
		 ...
		 var td = $(this);
		 var a=td.closest("tr").attr("id").slice(2);
		 var b=td.attr("id");
		 var txt = td.text();
		 ...
		 input.blur(function() {
			 ...
			 {
				 if (newtxt != txt) 
				 $.post("<%=basePath%>menu/saveEditParentMenuName.do",{"menuId":a,
					 "menuName":newtxt
				 });
				 td.html(newtxt);
			 }else{
				 if (newtxt != txt)$.post("<%=basePath%>menu/saveEditParentMenuOrder.do",{"menuId":a,
					 "menuOrder":newtxt
				 });
				 td.html(newtxt);
			 }
	...
	function editmenu(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑菜单";
		 diag.URL = '<%=basePath%>menu/toEdit.do?menuId='+menuId;
		 diag.Width = 223;
		 diag.Height = 256;
		 diag.CancelEvent = function(){ //关闭事件
	...
	function delmenu(menuId,isParent){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>menu/saveDelete.do?menuId="+menuId;			
...
<body>
<form action="menu/toEdit.do" name="menuForm" id="menuForm" method="post">
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty menuList}">
				...
				<td style="text-align:center"><a class="${menu.menuIcon == null ? 'icon-desktop' : menu.menuIcon}" onclick="editTb('${menu.menuId}')" ></a></td>
				<td class='center' id='MENU_NAME'>${menu.menuName }</td>
				<td>${menu.menuUrl == '#'? '': menuUrl}</td>
				<td class='center' id='MENU_ORDER'>${menu.menuOrder }</td>
				<td style="width: 25%;">
